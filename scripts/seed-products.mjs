...
// NOTE: We do NOT cleanup database here to persist seeded data
async function cleanup() {
  ...
  console.log("[seed-products] 服务已停止，数据保留在数据库中。");
}
...
try {
  ...
  console.log("[seed-products] 重置并同步数据库（prisma db push --force-reset）...");
  await runStep(
  ...
  console.log("[seed-products] 运行 Seed Playwright 测试...");
  await runStep("Playwright test", pnpmBin, ["exec", "playwright", "test", "--reporter=list", "apps/web/e2e/seed-products.spec.ts"], {
    env: {
      ...sharedEnv,
      PW_NO_WEBSERVER: "1",
    },
  ...
  console.error("[seed-products] 执行失败：", error);
  exitCode = 1;
} finally {
  await cleanup();
}
